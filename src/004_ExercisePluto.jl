...
# ╔═╡ 6bc06e62-b59e-4023-8f3e-e914042f168b
using Distributions, Statistics, CSV, DataFrames, Plots

# ╔═╡ 48fc9b20-2c7a-11ed-0843-53fdfe979d98
md"# Exercise 1.8
...
# ╔═╡ 913b4d06-6bae-4e5e-8435-2cd484491aac
md"### Data input"

# ╔═╡ bd59d43f-7fcb-4c2d-b64a-0972c9d453dd
mort = DataFrame(CSV.File("mortality.csv")) # mortality table

# ╔═╡ 203a993c-0c92-4f69-83d8-f063a3ae9e36
begin
	k = 1/10 # proportion of deaths caused by an accident
	q = Dict(mort.AGE .=> mort.qx) # Dictionary: a function that maps age -> qx
end;

# ╔═╡ a467ab99-2deb-4128-88f7-4a5fdc8ec083
policy = DataFrame(CSV.File("LIFEinsurance.csv")) # life insurance portfolio

# ╔═╡ 723cf3d0-a8ab-4933-83c2-e8a982f22dcf
md"### Individual risk model"

# ╔═╡ e1229c71-8ef6-4113-8399-a71feaa74ee1
md"For each insurance policy $j\in\{1,\ldots,r\}$ (in this example $r=10,000$) it is defined an indivual claim random variable $X_j$ as follows: 
...
# ╔═╡ aabe3d5b-36a7-4fe1-9ed6-0375179698c4
md"The theoretical mean and variance of $S$ accordingly to to the individual risk model, considering all $D_j$ and $A_j$ independent random variables, are given by:
...
# ╔═╡ d1be8ceb-b03f-401a-a1fd-a8b07382eaf0
md"Let's now define a random variable $N$ to count the number of claims: 
...
# ╔═╡ 068bc8be-f121-4b1c-9c23-251375320eba
begin
	ES = 0.0
	VS = 0.0
	EN = 0.0
...
# ╔═╡ fd2b15de-5c62-437f-85d6-929c5a7e8c28
begin
	# Version using `Bernoulli` from the `Distributions` package
	m = 100_000
	S = zeros(m)
	N = zeros(Int, m)
	# The macro @time measures execution time
    @time for j ∈ 1:r 
...
# ╔═╡ 8fe5ae20-d5cc-4942-a019-4ed05e6f4ecc
begin
...
# ╔═╡ 0f80eeeb-f10e-4908-b6f9-624ddefcd6a0
md"### Collective risk model"

# ╔═╡ 1515f60f-72b8-4b0e-b4a9-c56972eb93f9
md"The counting random variable $N$ represents the *frequency* (total number of claims in a given period) and the random variable $Y$ represents the *severity* of any claim (in monetary units). The **total claims** random variable is now given by: 
...
# ╔═╡ 30842862-ff2e-46ed-ab80-97f5622399d1
qN = EN / r

# ╔═╡ 7642d92f-d061-460c-9bb7-bb6cdbbc9ea4
md"Let's now compare the variance of a $\text{Binomial}(r, q_N)$ random variable and $\sigma_N^2$ obtained under the individual risk model:"

# ╔═╡ 2541fc9f-544d-4dde-bd8d-c423a00e545d
r * qN * (1 - qN), VN

# ╔═╡ 31158501-e943-4b6d-aa5f-96a63af00cb2
md"Not exactly the same but probably fair enough. Remember that the collective risk model is an *approximation.* In terms of standard deviation, this approximation will use a frequency with slightly more variability, but less that 0.1% higher:"

# ╔═╡ 5d840189-407a-4613-a81e-7ff0eb7db280
begin
	dif = 100 * (√(r * qN * (1 - qN) / VN) - 1) # in %
	println(round(dif, digits = 2), "%")
end

# ╔═╡ 61100030-ed7d-40f2-a731-151508d0b598
...
# ╔═╡ c4216930-d769-48ec-9f51-71e6c12d1570
begin
	n, rr = 35, 10_000
	prod(1 .- collect(1:(n-1)) ./ rr)
end

# ╔═╡ 55c678a2-e5ab-454c-ab39-ec064ad3434e
md"Eventhough in this case it would not be uniform sampling, the sampling probability for each policy is bounded above by:

$$\mathbb{P}(Z=c_j) \leq \frac{\max \{q_j:j=0,1,,\ldots,r\}}{\mathbb{E}(N)}$$"

# ╔═╡ 22aacb0c-9bd5-4527-944e-d529b063aac4
q[maximum(policy.AGE)] / EN

# ╔═╡ c8a4aa9e-f526-44b8-bbc2-e49b679c5bc4
md"With an average value of:"

# ╔═╡ fd395c2c-6266-4bad-8a62-5879999c4466
q[Int(round(mean(policy.AGE)))] / EN

# ╔═╡ 77275ef6-a4f2-4fff-b0b0-d43c5c6ebd09
md"And since the collective risk model is an approximation, we will assume for the severity that $Y_1, Y_2, \ldots$ are iid as a random variable $Y = Z(1+A)$ as defined above, and compare results with those obtained by the (more accurate) individual risk model:"

# ╔═╡ f4a5ff33-e9d8-495d-a7e0-0b5288f1b763
begin
	qq = zeros(r)
	for j ∈ 1:r
    	qq[j] = q[policy.AGE[j]]
...
# ╔═╡ a6cb626c-22ce-4908-87d7-c59cd0ba369a
...
# ╔═╡ 486f001b-1687-4c6c-bacc-0b7364729778
md"And finally, the collective risk model simulation:"
...
# ╔═╡ 86edf4eb-804c-4492-a47e-e0da83fa5a39
begin
...
# ╔═╡ da89eeaa-e74b-4512-9c33-8a0f73d6537c
md"Compare with individual risk model results:"

# ╔═╡ ab68f3c9-20c9-4df0-8d45-41fd288369df
begin
	println("sim M(S) = ", median(S))
	println("sim E(S) = ", mean(S))
	println("sim V(S) = ", var(S))
...
# ╔═╡ 76ecd657-d2de-47d6-a501-8d282ab6952d
md"In this case the collective risk model simulation is taking longer than the individual risk model because of the Categorical simulation from the insured amounts, so let's analyze the severity simulator:"

# ╔═╡ 745d6bb3-ada5-4e2a-914f-caa3cc4846ad
begin
	simY = simSeverity(100_000)
	histogram(simY, legend = false, color = :yellow,
		      xlabel = "claim amount", ylabel = "number of claims")
end

# ╔═╡ 0cec6cf5-45e1-4108-b006-7d3cefc28a18
2 * maximum(policy.INSAMOUNT)

# ╔═╡ 53428d88-6ca3-4532-81a9-a206f9aba880
md"Does not look like any of the well-known and well-behaved probability distributions we are aware of! This is because of the insured amount distribution in this portfolio:"

# ╔═╡ 9f6c5039-c92f-43d1-b945-eae07d55d719
histogram(policy.INSAMOUNT, legend = false, color = :green, xlabel = "insured amount", ylabel = "number of policies")

# ╔═╡ 0220f5ed-96f2-47ca-8c35-0a98c6c98bdb
md"What if we insist in fitting a well-known model for $Y$ in spite of this weird behaviour? Let's give it a try!"

# ╔═╡ 39dfdefe-30f7-4ec7-94d3-b5d0944cc361
begin
...
# ╔═╡ 507b38d6-5a02-4b27-a753-977b34480088
md"Doesn't look very well, uh? Let's use it and see what happens:"

# ╔═╡ 1a2bb3d9-5766-4a25-b504-b569a842818b
begin
	B2 = Binomial(r, qN)
	m2 = 100_000
	n2 = rand(B2, m2)
	Sc2 = zeros(m2)
	@time for i ∈ 1:m2
        Sc2[i] += sum(rand(fitY, n2[i]))
    end
end

# ╔═╡ 85952389-7989-44b2-bd80-f38e88a467fb
md"A lot faster, by far! But what about the results?"

# ╔═╡ 5a63e025-f3eb-4fec-9de9-a49b2f75090d
begin
	println("sim M(Sc) = ", median(Sc2))
	println("sim E(Sc) = ", mean(Sc2))
	println("sim V(Sc) = ", var(Sc2))
	println("sim VaR(0.995) = ", quantile(Sc2, 0.995))
end

# ╔═╡ 6baf2ee4-67de-4596-832e-7bbb091a85bc
md"Compare with individual risk model results:"

# ╔═╡ 47dc33ae-a236-4d90-8b58-8479283ae2bd
begin
	println("sim M(S) = ", median(S))
	println("sim E(S) = ", mean(S))
	println("sim V(S) = ", var(S))
...
# ╔═╡ c9229291-a759-4fad-a2fb-6aa409f6a726
md"Well, no surprise, right? Clearly it was not a good parametric fit for $Y$ but... what if we try a non-parametric simulation?"

# ╔═╡ 5a18431d-6eef-4a49-998f-9a4384cad9a9
begin
	simSeverityNP(n) = quantile(simY, rand(n))
	histogram(simSeverityNP(100_000), legend = false, color = :yellow,
		      xlabel = "claim amount", ylabel = "number of claims")
end

# ╔═╡ 4ca9d860-0b46-43e3-a5d2-97785e91a911
md"Bingo! Let's how fast it is in simulating the collective risk model:"

# ╔═╡ 38fbf04d-6f51-4ac6-b479-0f3fa69d2844
begin
	B3 = Binomial(r, qN)
	m3 = 1_000
	n3 = rand(B3, m3) # frequency
	Sc3 = zeros(m3)
	@time for i ∈ 1:m3
        Sc3[i] += sum(simSeverityNP(n3[i]))
    end
end

# ╔═╡ 985aee79-3072-469f-badc-feed60574063
md"Too slow just for 1,000 simulations! The problem is we used a very large number of simulations for `simY` (100,000) but to estimate a univariate density it would be more than enough with 1,000:"

# ╔═╡ bbf1ab3f-7de4-46da-9e17-5655b3a68928
begin
	simY4 = simSeverity(1_000)
	simSeverityNP4(n) = quantile(simY4, rand(n))	
	B4 = Binomial(r, qN)
	m4 = 100_000
	n4 = rand(B4, m4)
	Sc4 = zeros(m4)
	@time for i ∈ 1:m4
        Sc4[i] += sum(simSeverityNP4(n4[i]))
    end
end

# ╔═╡ 4d401206-47bc-4539-b63b-9e9d50179ca9
md"Wonderful time! Let's check the results:"

# ╔═╡ 742eb663-d2d5-4c81-806d-d04377bb5efa
begin
	println("sim M(Sc) = ", median(Sc4))
	println("sim E(Sc) = ", mean(Sc4))
	println("sim V(Sc) = ", var(Sc4))
	println("sim VaR(0.995) = ", quantile(Sc4, 0.995))
end

# ╔═╡ 0bcf28d6-3eea-43f0-ae98-52e0bb9ec440
md"Compare with individual risk model results:"

# ╔═╡ b65e50fc-e6a1-43b5-b6d4-8f1a2c316158
begin
	println("sim M(S) = ", median(S))
	println("sim E(S) = ", mean(S))
	println("sim V(S) = ", var(S))
...
# ╔═╡ 477e578d-3f62-493d-927a-a9310fffbf12
md"Fair enough! Faster and pretty close to the individual risk model, in spite of all the approximative assumptions."
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "e031a850730f4d0e81dabe72e654199f99ddbd99"
